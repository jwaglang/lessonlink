...
function ForgotPassword({
  resetState, setResetState, resetEmail, setResetEmail, onReset, loading, error, loginEmail,
}: {
  resetState: 'idle' | 'form' | 'sent';
  setResetState: (s: 'idle' | 'form' | 'sent') => void;
  resetEmail: string;
  ...
  if (resetState === 'sent') {
    return (
      <div className="mt-4 text-center space-y-1">
        <p className="text-sm font-medium">Check your inbox ✉️</p>
        <p className="text-xs text-muted-foreground">A reset link was sent to {resetEmail}</p>
        <button type="button" onClick={() => setResetState('idle')} className="text-xs text-primary hover:underline mt-1">Back to log in</button>
      </div>
    );
  }
  if (resetState === 'form') {
    return (
      <div className="mt-4 space-y-3 border-t pt-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setResetState('idle')} className="flex-1">Cancel</Button>
          <Button size="sm" onClick={onReset} disabled={loading || !resetEmail} className="flex-1">
  ...
  return (
    <div className="mt-3 text-center">
      <button
        type="button"
        onClick={() => { setResetEmail(loginEmail); setResetState('form'); }}
...
export default function LoginPage() {
  ...
  const [resetState, setResetState] = useState<'idle' | 'form' | 'sent'>('idle');
        ...
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <Tabs defaultValue="login" className="w-full" onValueChange={() => { setError(''); setSignupStep(1); }}>
            ...
            <TabsContent value="login">
              ...
              <ForgotPassword resetState={resetState} setResetState={setResetState} resetEmail={resetEmail} setResetEmail={setResetEmail} onReset={handleResetPassword} loading={loading} error={error} loginEmail={loginEmail} />
            </TabsContent>
