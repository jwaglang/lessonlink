    ...
    <div className="min-h-screen bg-background text-foreground">
      <style>{`
        @keyframes marbleBounce {
          ...
          12%  { transform: translateY(-44px); animation-timing-function: cubic-bezier(0.8,0,1,1); }
          24%  { transform: translateY(0);     animation-timing-function: cubic-bezier(0,0,0.2,1); }
          34%  { transform: translateY(-28px); animation-timing-function: cubic-bezier(0.8,0,1,1); }
          44%  { transform: translateY(0);     animation-timing-function: cubic-bezier(0,0,0.2,1); }
          52%  { transform: translateY(-16px); animation-timing-function: cubic-bezier(0.8,0,1,1); }
          60%  { transform: translateY(0);     animation-timing-function: cubic-bezier(0,0,0.2,1); }
          66%  { transform: translateY(-8px);  animation-timing-function: cubic-bezier(0.8,0,1,1); }
          72%  { transform: translateY(0);     animation-timing-function: cubic-bezier(0,0,0.2,1); }
          76%  { transform: translateY(-4px);  animation-timing-function: cubic-bezier(0.8,0,1,1); }
          80%  { transform: translateY(0);     animation-timing-function: cubic-bezier(0,0,0.2,1); }
          83%  { transform: translateY(-2px);  animation-timing-function: cubic-bezier(0.8,0,1,1); }
          86%  { transform: translateY(0); }
          100% { transform: translateY(0); }
        }
        .damped-bounce {
          animation: marbleBounce 7s linear forwards;
        }
      `}</style>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="flex items-center gap-2">
          <img src="/Logo Star Big.png" alt="Dork" className="w-8 h-8 object-contain" />
      ...
      <section className="min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20">
        <img src="/Logo Star Big.png" alt="Dork" className="w-24 h-24 object-contain mb-6 damped-bounce" />
      ...
      <section id="about" className="py-24 px-6 max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground text-lg">Native English speaker · Lisbon, Portugal</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-4 text-muted-foreground leading-relaxed">
            ...
            <p>
              I've worked with Oxford University Press, created educational YouTube series,
              and currently serve as a Cambridge University Speaking Examiner.
              Every lesson I teach draws on all of it.
            </p>
            <Link href="/">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: <Users className="h-6 w-6" />, value: '181', label: 'Students taught' },
              { icon: <Clock className="h-6 w-6" />, value: '25 yrs', label: 'Experience' },
              { icon: <Star className="h-6 w-6 fill-yellow-400 text-yellow-400" />, value: '5.0', label: 'Rating' },
              { icon: <Award className="h-6 w-6" />, value: '1,784', label: 'Lessons completed' },
            ].map((stat) => (
            ...
            <div className="col-span-2 rounded-xl border border-border bg-card p-4 text-sm text-muted-foreground space-y-1">
              ...
              <p>🎭 BFA — New York University</p>
              <p>📋 Cambridge Speaking Examiner (2016)</p>
              <p>📖 Oxford University Press Author</p>
            </div>
          </div>
        </div>

        <blockquote className="mt-12 text-center text-xl italic text-muted-foreground border-l-4 border-primary pl-6 max-w-2xl mx-auto text-left">
          "Jon has an incredible empathy and didactic with children. My son loves taking classes."
          <span className="block text-sm not-italic mt-2 text-muted-foreground/70">— Kiddoland parent</span>
        </blockquote>
      </section>
      ...
      <section id="program" className="py-24 px-6 bg-muted/30">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Dragon Levels */}
          <h3 className="text-2xl font-headline font-bold text-center mb-8 flex items-center justify-center gap-2">Enter the Dragon <img src="/Logo Star Big.png" alt="Dork" className="w-8 h-8 object-contain inline-block" /></h3>
      ...
      <footer className="border-t border-border py-8 px-6 text-center text-sm text-muted-foreground">
        ...
        <p>Powered by <Link href="/" className="text-primary hover:underline">LessonLink</Link> · Teacher Jon · Lisbon, Portugal</p>
      </footer>
    </div>
  );
}
