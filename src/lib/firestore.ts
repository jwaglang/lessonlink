...
export async function bookLesson(data: {
  ...
  // required for completion workflow (curriculum bookings)
  courseId: string;
  unitId: string;
  ...
  // NEW: billing mode (trial does NOT touch credit)
  billingType?: 'trial' | 'credit';
}): Promise<Lesson> {
  const billingType: 'trial' | 'credit' = data.billingType ?? 'credit';

  // Gate 1: curriculum permission (unit must be assigned) — only required for credit-backed bookings
  // (If you want trial to also require assignment, remove this if-block.)
  if (billingType === 'credit') {
    ...
    if (progressSnap.empty) {
      throw new Error('Cannot book: unit is not assigned (studentProgress missing).');
    }
  }

  // Gate 2: billing (credit must exist + have enough hours) — only for credit-backed bookings
  let creditDocRef: ReturnType<typeof doc> | null = null;
  if (billingType === 'credit') {
    ...
    if (creditSnap.empty) {
      throw new Error('Cannot book: no studentCredit found for this student/course (payment required).');
    }
    creditDocRef = creditSnap.docs[0].ref;
  }

  const instanceRef = doc(sessionInstancesCollection);

  await runTransaction(db, async (tx) => {
    // Reserve credit at booking time (uncommitted -> committed)
    if (billingType === 'credit') {
      if (!creditDocRef) throw new Error('Internal error: missing credit reference');

      const creditSnap = await tx.get(creditDocRef);
      if (!creditSnap.exists()) {
        throw new Error('Cannot book: studentCredit disappeared.');
      }

      const credit = creditSnap.data() as any;
      const uncommitted = Number(credit.uncommittedHours ?? 0);

      if (uncommitted < data.durationHours) {
        throw new Error(
          `Cannot book: insufficient credit. Available ${uncommitted}h, required ${data.durationHours}h.`
        );
      }

      tx.update(creditDocRef, {
        uncommittedHours: increment(-data.durationHours),
        committedHours: increment(data.durationHours),
        updatedAt: new Date().toISOString(),
      });
    }

    const newSessionInstanceData = {
      ...data,
      billingType, // stored on sessionInstance
    ...
    tx.set(instanceRef, newSessionInstanceData);
  });

  // Mark the availability slot as no longer available (keep existing behavior)
  ...
  // Return object shaped like Lesson
  return {
    id: instanceRef.id,
    ...data,
    billingType,
...
The new code will replace the old `bookLesson` function.
I will create the full file content and wrap it in the XML response.Right, this is a crucial update to make the booking process more robust by introducing a `billingType`. It correctly separates trial lessons from credit-based lessons and adds necessary checks for unit assignments and available credit, all within a secure transaction.

I will now replace the entire `bookLesson` function in `src/lib/firestore.ts` with your new implementation.

<changes>
  <description>Replaces the `bookLesson` function in `src/lib/firestore.ts` with a new version that supports a `billingType` ('trial' or 'credit'). The new function uses a Firestore transaction to ensure data integrity when reserving student credit and creating the session instance.</description>
  <change>
    <file>/src/lib/firestore.ts</file>
    <content><![CDATA[
